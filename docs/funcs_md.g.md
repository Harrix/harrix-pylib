...
- [🔧 Function `is_named_note_folder`](#-function-is_named_note_folder)
- [🔧 Function `is_note_in_named_folder`](#-function-is_note_in_named_folder)
- [🔧 Function `is_raw_markdown_enabled`](#-function-is_raw_markdown_enabled)
...
def generate_toc_with_links_content(markdown_text: str) -> str:
    ...
    if old_toc_position is not None:
        ...
        if old_toc_position >= len(lines):
            # One trailing newline: join() adds another → single blank line after </details>
            new_lines = [*lines, toc + "\n"]
        else:
            for i, line in enumerate(lines):
                if i == old_toc_position:
                    ...
                    # One trailing newline: join() adds another → single blank line after </details>
                    # (H012 forbids two consecutive empty lines.)
...
## 🔧 Function `get_set_variables_from_yaml`
...
## 🔧 Function `is_raw_markdown_enabled`
