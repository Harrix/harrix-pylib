...
from harrix_pylib.md_format.text_lines import _make_placeholder
...
def _decode_percent_encoded_url(url: str) -> str:
...
def _extract_link_destinations_from_text(body: str, *, start_index: int) -> tuple[str, list[_LinkDestination]]:
    ...
    return _scan_inline_links(body, handler), entries


def _format_inline_link_destination(destination: str) -> str:
    """Return canonical destination text for inline links and images."""
    url, title = _split_inline_destination(destination.strip())
...
def _formatted_href_from_placeholder(href: str, entries_by_index: dict[int, _LinkDestination]) -> str | None:
    ...
    if entry is None:
        return None
    url, _title = _split_inline_destination(_format_inline_link_destination(entry.destination))
    return url
